#!/usr/bin/env python3
"""Step-resolved HuggingFace Transformers profiler with an analytical roofline.

Transformers twin of step_metric5.py (the vLLM profiler). Everything that is
device/engine-independent is IDENTICAL to the vLLM script: the analytical
FLOP/byte model, the NVML power/utilization sampler, the idle-power baseline,
the drain-step marking, the CSV/summary/roofline outputs, and the gnuplot
figures. The two runs are therefore directly comparable - same prompt, same
models, same batch/gen-len, same metrics, same file formats - so you can drop
a Transformers run into --combine next to a vLLM run.

The ONE thing that changes is the measured loop. vLLM exposes one scheduler
step per engine.step(); Transformers does not, so we drive the phases by
hand with plain module forwards:

  * PREFILL = one forward over the whole batched prompt with use_cache=True.
    Produces the KV cache and the first token. This is step 0.
  * DECODE  = one forward of a single new token per sequence, feeding the
    growing past_key_values back in. Produces one token per sequence. These
    are steps 1..gen_len-1.

Each forward is wrapped in the same sync -> perf_counter_ns -> sync sandwich
and the same NVTX range ("prefill"/"decode") as the vLLM loop, so wall
latency is exactly that forward's GPU time and Nsight Compute can gate
capture per phase. Greedy argmax decoding matches vLLM temperature=0.0; we
never stop on EOS (vLLM ran with ignore_eos=True), so the run always yields
exactly gen_len token-producing steps and there is nothing to fold or drain.
The analytical FLOPs/bytes are unchanged, so achieved TFLOP/s = analytical
FLOPs / measured latency - lower than vLLM because eager Transformers is
slower, which is the whole point of the comparison.

Flags dropped vs step_metric5 (they were vLLM mechanism, not measurement):
  --kv-cache-gb, --gpu-mem-util, --no-enforce-eager, --max-model-len.
Flags added: --device (default: the model's first-parameter device) and
--attn-impl (eager/sdpa/flash_attention_2; default = model's own choice).
Transformers runs eager here - no CUDA graphs, no torch.compile - so each
step's kernels are individually visible and timeable, the same property
enforce_eager=True gave the vLLM run. Pass --attn-impl eager to also split
attention into per-op kernels for the ncu per-kernel path.

Setup (on the target NVIDIA system):
    pip install transformers accelerate torch pynvml
    sudo apt-get install gnuplot

Run (use ceilings measured by calibrate.py, not datasheet numbers):
    python3 step_metric_transformers.py --choice 1 --batch-size 4 \
        --gen-len 256 --peak-memory-gbs <measured> --peak-tflops <measured> \
        --out llama_hf_metrics

Model choices: 1=llama  2=gemma  3=gpt-oss (MoE)  4=qwen
Use a distinct --out from the vLLM run (e.g. *_hf_metrics) so the two sets of
CSV/roofline files do not overwrite each other.

Combine finished runs into one paper-style multi-panel figure (mix vLLM and
Transformers bases freely - the .dat/summary formats are identical):
    python3 step_metric_transformers.py \
        --combine llama_hf_metrics gemma_hf_metrics gpt_hf_metrics qwen_hf_metrics \
        --peak-memory-gbs <measured> --peak-tflops <measured> --out all_models_hf

Paper-exact per-kernel roofline (separate run; root needed for GPU perf
counters; expect tens of minutes - counter replay is slow). The NVTX ranges
are identical to the vLLM script, so the same recipe works; add
--attn-impl eager so attention shows up as its own kernels:
...
sm__sass_thread_inst_executed_op_hmul_pred_on.sum" \
        $(which python3) step_metric_transformers.py --choice 1 --gen-len 4 \
        --attn-impl eager --idle-baseline-s 0 --out ncu_scratch
...
# often have no reliable internet). Must be set before transformers/hf_hub are
# imported. Export HF_HUB_OFFLINE=0 to allow downloads.
...
# ---------------------------------------------------------------------------
# Analytical model (dense decoder) - IDENTICAL to step_metric5.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NVML power/utilization sampling - IDENTICAL to step_metric5.py
# ---------------------------------------------------------------------------
...
def measure_idle_power(seconds: float, interval_s: float, device_selector=None):
    """Mean NVML power over a quiescent window before the model is loaded."""
...
# ---------------------------------------------------------------------------
# Outputs - IDENTICAL to step_metric5.py
# ---------------------------------------------------------------------------
...
def mark_drain_like(records: list) -> list:
    ...
    The manual Transformers loop always runs a real forward per step, so this
    normally flags nothing - but it is kept identical to the vLLM script so
    the CSV/summary schema and the reprocess path match exactly. Rows are
    only FLAGGED (drain_like=True): they stay in the CSV for investigation;
    summary stats and the roofline .dat leave them out."""
    for r in records:
        r["drain_like"] = False
...
def reprocess(base: str, peak_tflops, peak_gbs) -> int:
    ...
    No GPU, no model: applies mark_drain_like to a run recorded before the
    fix existed, so any token-carrying drain step drops out of the stats and
    plots. The CSV itself is never rewritten - raw data stays exactly as
    measured."""
    records = []
...
def parse_args():
    ...
    p.add_argument("--device", default=None,
                   help="torch device (e.g. cuda:0). Default: load with "
                        "device_map='auto' and use the model's first-"
                        "parameter device")
    p.add_argument("--attn-impl", default=None,
                   choices=["eager", "sdpa", "flash_attention_2"],
                   help="attn_implementation passed to from_pretrained. "
                        "Default = model's own choice (usually sdpa). Use "
                        "eager to split attention into per-op kernels for "
                        "the ncu per-kernel roofline path")
    ...
    p.add_argument("--idle-baseline-s", type=float, default=10.0,
                   help="seconds of NVML idle-power sampling before the model "
                        "loads; subtracted to report dynamic power/energy "
    ...
    p.add_argument("--reprocess", action="store_true",
                   help="skip profiling; re-derive <--out>_summary.json + "
                        ...
                        "applying the drain-step marking. The CSV is not "
                        "rewritten")
    p.add_argument("--out", default="hf_step_metrics")
    args = p.parse_args()
...
def main() -> int:
    ...
    if args.model:
        ...
        is_moe = spec["moe"]
        use_processor = spec.get("use_processor", False)
    ...
    # Idle-power baseline: before the model is loaded, so the GPU is quiescent.
    idle_power_w = None
    ...
    torch_dtype = getattr(torch, DTYPE_MAP.get(args.dtype, "bfloat16"))

    def sync():
        if torch.cuda.is_available():
            torch.cuda.synchronize()

    def nvtx_push(label):  # no-op off CUDA so a CPU smoke test still runs
        if torch.cuda.is_available():
    ...
    # Tokenizer / processor (Gemma multimodal ships a processor). Matches the
    # project's metrics_transformers.py loading conventions.
    if use_processor:
        tok = AutoProcessor.from_pretrained(model_id, trust_remote_code=True)
    else:
        tok = AutoTokenizer.from_pretrained(model_id, trust_remote_code=True)
    underlying = getattr(tok, "tokenizer", tok)
    if (getattr(underlying, "pad_token", None) is None
            and getattr(underlying, "eos_token", None) is not None):
    ...
    ct_kwargs = {"add_generation_prompt": True, "tokenize": False}
    if hasattr(tok, "parse_response"):  # Gemma processor: skip thinking block
        ct_kwargs["enable_thinking"] = False
    text = tok.apply_chat_template(
        [{"role": "system", "content": SYSTEM_PROMPT},
         {"role": "user", "content": USER_PROMPT}], **ct_kwargs)
    # the rendered template already contains BOS; don't add it twice
    ids = list(underlying(text, add_special_tokens=False).input_ids)
    ...
    # Model load. enforce eager execution (no CUDA graphs / torch.compile) so
    # each step's kernels are individually visible, the property the vLLM run
    # got from enforce_eager=True.
    ...
    print(f"[info] model on {device}, dtype {torch_dtype}, "
          f"attn_impl {args.attn_impl or 'default'}")

    # Batched prompt: the same prompt token IDs for every sequence, so prefill
    # processes n_prompt*batch tokens in one forward and each decode emits one
    # token per sequence - the exact per-step token accounting of the vLLM run.
    input_ids = torch.tensor([ids] * args.batch_size, device=device)
    base_mask = torch.ones_like(input_ids)

    def greedy_next(logits):
        # last-position logits -> next token id per sequence (temperature 0)
        return logits[:, -1, :].argmax(dim=-1)

    # Untimed warmup: triggers kernel JIT/autotune and lazy allocations. The
    # NVTX range keeps these kernels out of --nvtx-include captures. Runs the
    # same prefill+decode path as the measured loop, just short.
    warm_steps = max(1, min(8, args.gen_len))
    with torch.inference_mode():
        nvtx_push("warmup")
        out = model(input_ids=input_ids, attention_mask=base_mask,
                    use_cache=True)
        past = out.past_key_values
        nxt = greedy_next(out.logits)
        mask = base_mask
        for _ in range(warm_steps - 1):
            mask = torch.cat(
                [mask, torch.ones((args.batch_size, 1), dtype=mask.dtype,
                                  device=device)], dim=1)
            out = model(input_ids=nxt.unsqueeze(1), attention_mask=mask,
                        past_key_values=past, use_cache=True)
            past = out.past_key_values
            nxt = greedy_next(out.logits)
    del out, past, nxt, mask
    sync()
    ...
    try:
        with torch.inference_mode():
            ...
            nvtx_push("prefill")  # gates ncu --nvtx-include "prefill/"
            out = model(input_ids=input_ids, attention_mask=base_mask,
                        use_cache=True)
            nvtx_pop()
            sync()
            ...
            # ---- DECODE: steps 1..gen_len-1 (one new token per sequence) ----
            mask = base_mask
            kv_len = n_prompt
            for step in range(1, args.gen_len):
                mask = torch.cat(
                    [mask, torch.ones((args.batch_size, 1), dtype=mask.dtype,
                                      device=device)], dim=1)
                cur = nxt.unsqueeze(1)
                sync()
                t0 = time.perf_counter_ns()
                nvtx_push("decode")  # gates ncu --nvtx-include "decode/"
                ...
                records.append({
                    "step": step, "phase": "decode", "tokens": tokens,
                    "kv_len": kv_len, "start_ns": t0, "end_ns": t1,
                    "latency_s": latency,
                    "flops": int(flops), "bytes": int(bytes_moved),
                    "tokens_per_second": tokens / latency,
                    "ai_flops_per_byte": flops / bytes_moved,
    ...
    # Prefill emits the first token, so gen_len token-producing steps total.
    if len(records) != args.gen_len:
        print(f"[warn] {len(records)} token-producing steps but expected "
              f"{args.gen_len}; phase accounting for this run may be off")
    mark_drain_like(records)
    ...
    summary["config"] = {
        ...
        "prompt_tokens": n_prompt, "engine": "transformers",
